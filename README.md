...
runs in a datacenter and has no access to a visitor's local files, since browsers don't allow
...
in a native app window (via `pywebview`), so no browser is required.
...
```
The finished executable is at `dist/IntegrityMonitor.exe`, a single file that needs no Python
...
Right now `IntegrityMonitor.exe` is a single portable executable, so users can just double-click
